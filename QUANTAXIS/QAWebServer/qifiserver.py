...
@lru_cache(maxsize=2)
def get_qifis_manager(model):
    return QA_QIFISMANAGER(mongo_uri, model=model)
...
class QAQIFI_Handler(QABaseHandler):
    def get(self):
        ...
        if action == 'acchistory':
            ...
            res = get_qifis_manager('BACKTEST').get_holding_panel(acc, trading_day)
            self.write({'res': QA_util_to_json_from_pandas(res)})
...
class QAQIFIS_Handler(QABaseHandler):
    def get(self):
        action = self.get_argument('action', 'acchistory')
        manager = get_qifis_manager('BACKTEST')

        if action == 'accountlist':
            res = manager.get_allaccountname()
            self.write({'res': res})
        elif action == 'portfoliolist':
            res = manager.get_allportfolio()
            self.write({'res': res})
        elif action == 'accountinportfolio':
            portfolio = self.get_argument('portfolio', 't12')
            res = manager.get_portfolio_panel(portfolio)
    ...
    def post(self):
        ...
        if action == 'drop_account':
            ...
            res = manager.drop_account(account_cookie)
            self.write({
                'res': res,
                'status': 200
            })
            ...
            res = manager.drop_many(account_cookies)
            self.write({
                'res': res,
                'status': 200
            })
...
class QAQIFIS_REALTIME_Handler(QABaseHandler):
    def get(self):
        action = self.get_argument('action', 'acchistory')
        manager = get_qifis_manager('REALTIME')
